function tempTrack = compute_angle_between_keypoints(mouseData, shockFrames, keypts, node_name1, node_name2, node_name3,tempTrack)
% tempTrack = compute_angle_between_keypoints(mouseData, shockFrames, keypts)
%
...
%   node_name1, node_name2, node_name3: string assocaited with each node,
%   i.e., "upper_eye", case sensitive
...
keypt_index1 = find(strcmp(keypts,node_name1));
keypt_index2 = find(strcmp(keypts,node_name2));
keypt_index3 = find(strcmp(keypts,node_name3));
...
if numel(keypt_index1)==1
    ...
    tmp.(node_name1) =  mouseData.tracks(shockFrames,keypt_index1,:);
    keypt_1 = squeeze(mean(tmp.(node_name1),2));
    fn1 = node_name1;
end
...
if numel(keypt_index2)==1
    ...
    % compute the centroid
    tmp.(node_name2) =  mouseData.tracks(shockFrames,keypt_index2,:);
    keypt_2 = squeeze(mean(tmp.(node_name2),2));
    fn2 = node_name2;
end
...
if numel(keypt_index3)==1
    ...
    % compute the centroid
    tmp.(node_name3) =  mouseData.tracks(shockFrames,keypt_index3,:);
    keypt_3 = squeeze(mean(tmp.(node_name3),2));
    fn3 = node_name3;
end
